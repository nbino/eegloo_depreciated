...
var MOOdalBox = new Class ({
	...
	initialize: function (options) {
		...
		this.overlayFrame = new Element('iframe').setOpacity(0).setStyles({position: 'absolute', border: '3px solid #FFF', width: '90%'}).injectInside(document.body);
		...
		this.clearDiv = new Element('div').setStyle('clear', 'both').injectInside(this.bottom);
	...
	position: function() {
		...
		this.overlayFrame.setStyles({top: Window.getScrollTop()+'px', height: Window.getHeight()+'px', display: 'block'}).setOpacity(0.01);
	},
	...
	setup: function(open) {
		...
		if (window.gecko && open) {
			this.elements2hide = $A(document.getElementsByTagName('embed'));	
		}

			this.elements2hide.each(function(el){
