class ListingsController < ApplicationController
  ...
  def search
    ...
    params[:listing_info].delete_if {|key, val| val=="0" || val==""}
  ...
  def load_all_dropdown_values
    ...
    #no need for 'shared' and '1'
    @no_of_bathrooms_select = BathroomN.find :all, :conditions=>"name in ('2','3')"
  ...
  def load_basic_dropdown_values
    @apt_types = AptType.find :all
    @nhoods = Nhood.find :all
    @rent_ranges = RentRange.find :all
  end
