module PaginatingFind
  module Helpers
    DEFAULT_OPTIONS = { :name => :page, :window_size => 2, 
      ...
      :params => {}, :auto_submit_form => false, :set_hidden_field => false }
    ...
    def remote_paginating_links(paginator, options = {}, html_options = {})
      paginating_links_each(paginator, options) do |n|
        ...
        js = ''
        js << ";document.forms['#{options[:form_name]}'].elements['#{options[:hidden_field_name]}'].value = #{n};" if options[:set_hidden_field] && options[:form_name] && options[:hidden_field_name]
        js << ";document.forms['#{options[:form_name]}'].onsubmit();" if options[:auto_submit_form] && options[:form_name] 
        
        html_options[:onclick] = "javascript:#{js};return(false);"
        
        
        link_to(n, '#', html_options)
      end
    end
